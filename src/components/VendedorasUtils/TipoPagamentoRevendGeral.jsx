...
import { ConsultaTipoPagVendedoraGeral } from '../funcionalidades/bancoGet'
...
const LinhaTabela = (props)=>{
    const {val,i} = props
        return <>
          <tr key={i}>
            {val.ven_nome ? <td>{val.ven_nome}</td> : <></>}
...
export default function TipoPagamentosVendedoras(props){
    ...
    pesquisaTipoPagVend: 1, alerta: {status: false, title: '', text: ''}, graficoConfig: {}, selectedData: null, loja: ''})
    ...
    useEffect(()=>{
        if (stateInfo.pesquisaTipoPagVend > 0 && stateInfo.dataInit !== ''){
            ConsultaTipoPagVendedoraGeral({DATAINIT : stateInfo.dataInit, DATAFIM: stateInfo.dataFim, 
                CODIGOVEND: stateInfo.codigoVend, LOJAORIGEM: stateInfo.loja})
                .then(resp => {
                    if(resp.Error){
    ...
    flexDirection: 'column', overflowY: "hidden"}}>
        ...
        <div className="card card--accent">
            ...
            <div className="inputsCard">
                ...
                <div className='inputTitleComp'>
                    <label>Loja</label>
                    <input type="text" id="loja" onChange={(e)=>dispachInfo({type:'loja', value: e.target.value})} value={stateInfo.loja}/>
                </div>
      ...
      : <></>}
        ...
        <table className="tablePagVendedoras" ref={tablePagVendedoras}>  
            <thead>
                {stateInfo.dadosGraf ? <tr>
                    {((stateInfo.dadosGraf.POSITIVO[0] && stateInfo.dadosGraf.POSITIVO[0].ven_nome)
        ...
        <tbody>
            {stateInfo.dadosGraf ? stateInfo.dadosGraf.POSITIVO.map((val, i) => {
                return <LinhaTabela  val={val} i={i} key={"celulaTipoPagVend"+i}/>
                }) : <></>}
